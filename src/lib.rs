...
impl OutlineBuilder for ReshapedBuilder<'_> {
    ...
    fn quad_to(&mut self, x1: f32, y1: f32, x: f32, y: f32) {
        use std::fmt::Write;
        write!(self.0, "Q {} {} {} {} ", x1 - self.2, self.1 - y1 - self.3, x - self.2, self.1 - y - self.3).unwrap()
    }

    fn curve_to(&mut self, x1: f32, y1: f32, x2: f32, y2: f32, x: f32, y: f32) {
        use std::fmt::Write;
        write!(self.0, "C {} {} {} {} {} {} ", x1 - self.2, self.1 - y1 - self.3, x2 - self.2, self.1 - y2 - self.3, x - self.2, self.1 - y - self.3).unwrap()
    }
...
fn normalize(str: &str) -> String {
    ...
    let tmp = tmp.get(tmp.len() - 1).unwrap_or(tmp.last().unwrap());

    let tmp = if tmp.starts_with("fa-") {
        tmp[3..].to_string()
    } else {
        tmp.to_string()
    };
...
fn font_to_svg(face: &mut Face) -> Vec<(String, String)> {
    ...
    for id in 0..face.number_of_glyphs() {
        ...
        path_buf.clear();
        let mut reshaped_builder = ReshapedBuilder(&mut path_buf, (bbox.y_max + bbox.y_min) as f32, bbox.x_min as f32, bbox.y_min as f32);
...
pub async fn scrap_font_icons<T: IntoUrl>(css_url: T) -> reqwest::Result<Vec<(String, String)>> {
    let mut output = Vec::new();
    let css_url = css_url.into_url().unwrap();

    let css_file = reqwest::get(css_url.clone()).await?.text().await?;
